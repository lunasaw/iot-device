...
public class HelloWorld {
    ...
    public void init(final DeviceInfoData deviceInfoData) {
        ...
        if (!deviceInfoData.instanceId.isEmpty()) {
            ...
            config.channelHost = "ssl://" + deviceInfoData.instanceId + ".mqtt.iothub.aliyuncs.com:8883";
        } else {
            // 如果实例详情页面没有实例的id, 建议开发者填入实例所在的region. 注：该用法不支持深圳和北京两个region
            config.channelHost = deviceInfoData.productKey + ".iot-as-mqtt." + deviceInfoData.region + ".aliyuncs.com:8883";
        }
    ...
    public void deviceRegister(DeviceInfoData deviceInfoData) {
        ...
        if (!deviceInfoData.instanceId.isEmpty()) {
            // 如果实例详情页面有实例的id, 建议开发者填入实例id. 推荐的做法
            MqttConfigure.mqttHost = "ssl://" + deviceInfoData.instanceId + ".mqtt.iothub.aliyuncs.com:8883";
        } else {
            // 如果实例详情页面没有实例的id, 建议开发者填入实例所在的region. 注：该用法不支持深圳和北京两个region
            MqttConfigure.mqttHost = deviceInfoData.productKey + ".iot-as-mqtt." + deviceInfoData.region + ".aliyuncs.com:8883";
        }

        final MqttInitParams initParams =
            new MqttInitParams(deviceInfoData.productKey, deviceInfoData.productSecret, deviceInfoData.deviceName, "", registerType);
        ...
        LinkKit.getInstance().deviceDynamicRegister(initParams, new IOnCallListener() {
            ...
            public void onFailed(ARequest aRequest, com.aliyun.alink.linksdk.channel.core.base.AError aError) {
                System.out.println(JSONObject.toJSONString(aError));
        ...
        try {
            ...
            // 清空注册的实例信息
            MqttConfigure.registerInstanceId = null;
            ...
            public void onResponse(CommonRequest commonRequest, CommonResponse commonResponse) {
                ...
                try {
                    ResponseModel<Map<String, String>> response =
                        new Gson().fromJson(commonResponse.getData(), new TypeToken<ResponseModel<Map<String, String>>>() {}.getType());
